<template>
	<view class="page-waterelectricity-apply">
		<view class="tips">
			...
			<image src="../../indexPages/static/icon_phone.png" style="width: 26rpx; height: 24rpx;margin-left: 22rpx;" mode="" />
		</view>
...
<script>
	...
	export default{
		...
		methods:{
			...
			// 拨号
			handDial() {
				if(this.electricityPhone){
					uni.showActionSheet({
						itemList: [`客服：${this.electricityPhone}`, '拨打'],
						success: (res) => {
							if (res.tapIndex == 1) {
								uni.makePhoneCall({
									phoneNumber: this.electricityPhone 
								});
							}
						},
					});
				...
				api.my.getMyHousingPages(canshu).then(res => {
					if (res.status !== '0') {
						...
						if (res.data.records.length > 0) {
							...
							_this.floorName = _this.homeList.communityName + _this.homeList.buildingNumber + '栋' + _this
			...
			async getPropertyPhone() {
				const query = {
					phoneType: 'electricity',
				}
				const res = await api.service.getPropertyPhone(query)
				if (res.status !== '0') {
					return
				} else {
					this.electricityPhone = res.data.electricityPhone[0]
				}
			},
			bindDateChanges(e){
